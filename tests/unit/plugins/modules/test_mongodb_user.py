# -*- coding: utf-8 -*-
"""Comprehensive unit tests for mongodb_user module."""
...
from ansible_collections.stevefulme1.mongodb.plugins.modules import mongodb_user
...
class TestDocumentation:
    ...
    def test_documentation_exists(self):
        assert hasattr(mongodb_user, "DOCUMENTATION")
        assert len(mongodb_user.DOCUMENTATION) > 0

    def test_documentation_has_module_name(self):
        assert "mongodb_user" in mongodb_user.DOCUMENTATION or "user" in mongodb_user.DOCUMENTATION

    def test_documentation_has_short_description(self):
        assert "short_description" in mongodb_user.DOCUMENTATION

    def test_documentation_has_options(self):
        assert "options" in mongodb_user.DOCUMENTATION

    def test_examples_exist(self):
        assert hasattr(mongodb_user, "EXAMPLES")
        assert len(mongodb_user.EXAMPLES) > 0

    def test_examples_contain_fqcn(self):
        assert "stevefulme1.mongodb" in mongodb_user.EXAMPLES

    def test_return_exists(self):
        assert hasattr(mongodb_user, "RETURN")
        assert len(mongodb_user.RETURN) > 0
...
class TestCreate:
    ...
    def test_create_with_all_params(self, mock_client):
        params = {"name": "full-user", "description": "test"}
