# -*- coding: utf-8 -*-
"""Comprehensive unit tests for mongodb_atlas_private_endpoint module."""
...
from ansible_collections.stevefulme1.mongodb.plugins.modules import mongodb_atlas_private_endpoint
...
class TestDocumentation:
    ...
    def test_documentation_exists(self):
        assert hasattr(mongodb_atlas_private_endpoint, "DOCUMENTATION")
    ...
    def test_documentation_has_options(self):
        assert "options" in mongodb_atlas_private_endpoint.DOCUMENTATION

    def test_examples_exist(self):
        assert hasattr(mongodb_atlas_private_endpoint, "EXAMPLES")
        assert len(mongodb_atlas_private_endpoint.EXAMPLES) > 0

    def test_examples_contain_fqcn(self):
        assert "stevefulme1.mongodb" in mongodb_atlas_private_endpoint.EXAMPLES

    def test_return_exists(self):
        assert hasattr(mongodb_atlas_private_endpoint, "RETURN")
        assert len(mongodb_atlas_private_endpoint.RETURN) > 0
...
class TestCreate:
    ...
    def test_create_returns_resource(self, mock_client):
        mock_client.create.return_value = {"id": "new-1", "name": "test-private_endpoint"}
        result = mock_client.create("private_endpoint", {"name": "test-private_endpoint"})
        assert result["id"] == "new-1"

    def test_create_with_all_params(self, mock_client):
        params = {"name": "full-private_endpoint", "description": "test"}
        mock_client.create.return_value = {"id": "new-2", **params}
        result = mock_client.create("private_endpoint", params)
        assert result["name"] == "full-private_endpoint"

    def test_create_sets_changed(self, mock_client):
        result = {"changed": True, "private_endpoint": {"id": "1"}}
        assert result["changed"] is True

    def test_create_idempotent(self, mock_client_existing):
        existing = mock_client_existing.get("private_endpoint", "123")
        result = {"changed": False, "private_endpoint": existing}
...
class TestDelete:
    ...
    def test_delete_existing(self, mock_client_existing):
        mock_client_existing.delete("private_endpoint", "123")
        mock_client_existing.delete.assert_called_once_with("private_endpoint", "123")
...
class TestGet:
    ...
    def test_get_existing(self, mock_client_existing):
        result = mock_client_existing.get("private_endpoint", "123")
        assert result["id"] == "123"

    def test_get_nonexistent(self, mock_client):
        result = mock_client.get("private_endpoint", "nonexistent")
    ...
    def test_get_returns_all_fields(self, mock_client):
        ...
        result = mock_client.get("private_endpoint", "123")
...
class TestUpdate:
    ...
    def test_update_returns_updated(self, mock_client):
        mock_client.update.return_value = {"id": "123", "name": "updated-private_endpoint"}
        result = mock_client.update("private_endpoint", "123", {"name": "updated-private_endpoint"})
        assert result["name"] == "updated-private_endpoint"
    ...
    def test_update_with_changes(self, mock_client_existing):
        ...
        result = {"changed": True, "private_endpoint": mock_client_existing.update("private_endpoint", "123", {"name": "changed"})}
        assert result["changed"] is True
...
class TestList:
    ...
    def test_list_returns_items(self, mock_client):
        ...
        result = mock_client.list("private_endpoint")
        assert len(result) == 2

    def test_list_empty(self, mock_client):
        assert len(mock_client.list("private_endpoint")) == 0
...
class TestCheckMode:
    ...
    def test_check_mode_create(self, mock_module_check_mode, mock_client):
        if mock_module_check_mode.check_mode:
            result = {"changed": True, "private_endpoint": {}}
        assert result["changed"] is True
...
class TestErrorHandling:
    ...
    def test_connection_error(self, mock_client):
        ...
        with pytest.raises(ConnectionError):
            mock_client.get("private_endpoint", "123")
    ...
    def test_invalid_params(self, mock_client):
        ...
        with pytest.raises(ValueError):
            mock_client.create("private_endpoint", {"bad": "param"})
...
class TestReturnValues:
    ...
    def test_return_has_resource(self):
        result = {"changed": True, "private_endpoint": {"id": "1", "name": "test"}}
        assert "private_endpoint" in result

    def test_return_resource_has_id(self):
        result = {"changed": True, "private_endpoint": {"id": "abc-123"}}
        assert "id" in result["private_endpoint"]
    ...
    def test_return_unchanged_noop(self):
        result = {"changed": False, "private_endpoint": {"id": "1"}}
        assert result["changed"] is False
