# -*- coding: utf-8 -*-
"""Comprehensive unit tests for mongodb_atlas_user module."""
...
from ansible_collections.stevefulme1.mongodb.plugins.modules import mongodb_atlas_user
...
class TestDocumentation:
    ...
    def test_documentation_exists(self):
        assert hasattr(mongodb_atlas_user, "DOCUMENTATION")
        assert len(mongodb_atlas_user.DOCUMENTATION) > 0

    def test_documentation_has_module_name(self):
        assert "mongodb_atlas_user" in mongodb_atlas_user.DOCUMENTATION or "atlas_user" in mongodb_atlas_user.DOCUMENTATION

    def test_documentation_has_short_description(self):
        assert "short_description" in mongodb_atlas_user.DOCUMENTATION

    def test_documentation_has_options(self):
        assert "options" in mongodb_atlas_user.DOCUMENTATION

    def test_examples_exist(self):
        assert hasattr(mongodb_atlas_user, "EXAMPLES")
        assert len(mongodb_atlas_user.EXAMPLES) > 0

    def test_examples_contain_fqcn(self):
        assert "stevefulme1.mongodb" in mongodb_atlas_user.EXAMPLES

    def test_return_exists(self):
        assert hasattr(mongodb_atlas_user, "RETURN")
        assert len(mongodb_atlas_user.RETURN) > 0
...
class TestCreate:
    ...
    def test_create_returns_resource(self, mock_client):
        mock_client.create.return_value = {"id": "new-1", "name": "test-atlas_user"}
        result = mock_client.create("atlas_user", {"name": "test-atlas_user"})
        assert result["id"] == "new-1"

    def test_create_with_all_params(self, mock_client):
        params = {"name": "full-atlas_user", "description": "test"}
        mock_client.create.return_value = {"id": "new-2", **params}
        result = mock_client.create("atlas_user", params)
        assert result["name"] == "full-atlas_user"

    def test_create_sets_changed(self, mock_client):
        result = {"changed": True, "atlas_user": {"id": "1"}}
        assert result["changed"] is True

    def test_create_idempotent(self, mock_client_existing):
        existing = mock_client_existing.get("atlas_user", "123")
        result = {"changed": False, "atlas_user": existing}
...
class TestDelete:
    ...
    def test_delete_existing(self, mock_client_existing):
        mock_client_existing.delete("atlas_user", "123")
        mock_client_existing.delete.assert_called_once_with("atlas_user", "123")
...
class TestGet:
    ...
    def test_get_existing(self, mock_client_existing):
        result = mock_client_existing.get("atlas_user", "123")
        assert result["id"] == "123"

    def test_get_nonexistent(self, mock_client):
        result = mock_client.get("atlas_user", "nonexistent")
    ...
    def test_get_returns_all_fields(self, mock_client):
        ...
        result = mock_client.get("atlas_user", "123")
...
class TestUpdate:
    ...
    def test_update_returns_updated(self, mock_client):
        mock_client.update.return_value = {"id": "123", "name": "updated-atlas_user"}
        result = mock_client.update("atlas_user", "123", {"name": "updated-atlas_user"})
        assert result["name"] == "updated-atlas_user"
    ...
    def test_update_with_changes(self, mock_client_existing):
        ...
        result = {"changed": True, "atlas_user": mock_client_existing.update("atlas_user", "123", {"name": "changed"})}
        assert result["changed"] is True
...
class TestList:
    ...
    def test_list_returns_items(self, mock_client):
        ...
        result = mock_client.list("atlas_user")
        assert len(result) == 2

    def test_list_empty(self, mock_client):
        assert len(mock_client.list("atlas_user")) == 0
...
class TestCheckMode:
    ...
    def test_check_mode_create(self, mock_module_check_mode, mock_client):
        if mock_module_check_mode.check_mode:
            result = {"changed": True, "atlas_user": {}}
        assert result["changed"] is True
...
class TestErrorHandling:
    ...
    def test_connection_error(self, mock_client):
        ...
        with pytest.raises(ConnectionError):
            mock_client.get("atlas_user", "123")
    ...
    def test_invalid_params(self, mock_client):
        ...
        with pytest.raises(ValueError):
            mock_client.create("atlas_user", {"bad": "param"})
...
class TestReturnValues:
    ...
    def test_return_has_resource(self):
        result = {"changed": True, "atlas_user": {"id": "1", "name": "test"}}
        assert "atlas_user" in result

    def test_return_resource_has_id(self):
        result = {"changed": True, "atlas_user": {"id": "abc-123"}}
        assert "id" in result["atlas_user"]
    ...
    def test_return_unchanged_noop(self):
        result = {"changed": False, "atlas_user": {"id": "1"}}
        assert result["changed"] is False
